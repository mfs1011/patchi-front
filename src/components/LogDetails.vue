        ...
        <TabPanels pt:root="px-0 pb-0">
            ...
            >
                ...
                >
                    ...
                    <Column
                        v-if="entityType === 'Inventory' || entityType === 'TransferInvoice' || entityType === 'WriteOffInvoice'"
                    ...
                    <Column
                        v-if="entityType === 'Inventory'|| entityType === 'TransferInvoice' || entityType === 'WriteOffInvoice'"
                        field="expiryDate" :header="t('labels.expiryDate')"
                    >
                        <template #body="{ data }">
                            <Skeleton height="2rem" v-if="isLoading"/>
                    ...
                    <Column v-if="entityType === 'TransferInvoice' || entityType === 'WriteOffInvoice'" field="category" :header="t('labels.category')">
                        <template #body="{ data }">
                            <Skeleton height="2rem" v-if="isLoading"/>
                            <p v-else>{{ data.locationQuantity?.product.category.name }}</p>
                        </template>
                    </Column>
                    <Column v-if="entityType === 'TransferInvoice' || entityType === 'WriteOffInvoice'" field="type" :header="t('labels.type')">
                        <template #body="{ data }">
                            <Skeleton height="2rem" v-if="isLoading"/>
                            <p v-else>{{ data.locationQuantity?.product.category.categoryType.name }}</p>
                        </template>
                    </Column>
                    <Column v-if="entityType === 'TransferInvoice' || entityType === 'WriteOffInvoice'" field="amount" :header="t('labels.amount')">
                        <template #body="{ data }">
                            <Skeleton height="2rem" v-if="isLoading"/>
                            <p v-else>{{ formatCurrency(data.qty) }} {{ t(`labels.${data.locationQuantity?.product.category.unit.name}`) }}</p>
                        </template>
                    </Column>
                    <Column v-if="entityType === 'TransferInvoice' || entityType === 'WriteOffInvoice'" field="costPrice" :header="t('labels.costPrice')">
                        <template #body="{ data }">
                            <Skeleton height="2rem" v-if="isLoading"/>
                    ...
                    <Column v-if="entityType === 'Kit'" field="wholesalePrice" :header="t('labels.wholesalePrice')">
                        <template #body="{ data }">
                            <Skeleton height="2rem" v-if="isLoading"/>
                            <p v-else>{{ formatCurrency(data.product.wholesalePrice) }}$</p>
                        </template>
                    </Column>
                    <Column v-if="entityType === 'Kit'" field="retailPrice" :header="t('labels.retailPrice')">
                        <template #body="{ data }">
                            <Skeleton height="2rem" v-if="isLoading"/>
                ...
                >
                    ...
                    <Column v-if="entityType === 'TransferInvoice' || entityType === 'WriteOffInvoice'" field="name" :header="t('labels.title')">
                        <template #body="{ data }">
                            <Skeleton height="2rem" v-if="isLoading"/>
                            <p v-else>{{ data.locationQuantity.product.name }}</p>
                        </template>
                    </Column>
                    <Column v-if="entityType === 'TransferInvoice' || entityType === 'WriteOffInvoice'" field="code" :header="t('labels.code')">
                        <template #body="{ data }">
                            <Skeleton height="2rem" v-if="isLoading"/>
                            <p v-else>{{ data.locationQuantity.product.code }}</p>
                        </template>
                    </Column>
                    <Column v-if="entityType === 'TransferInvoice' || entityType === 'WriteOffInvoice'" field="color" :header="t('labels.color')">
                        <template #body="{ data }">
                            <Skeleton height="2rem" v-if="isLoading"/>
                            <p v-else>{{ data.locationQuantity.color ? data.color.name : '-' }}</p>
                        </template>
                    </Column>
                    <Column v-if="entityType === 'TransferInvoice' || entityType === 'WriteOffInvoice'" field="category" :header="t('labels.category')">
                        <template #body="{ data }">
                    ...
                    <Column v-if="entityType === 'TransferInvoice' || entityType === 'WriteOffInvoice'" field="expiryDate" :header="t('labels.expiryDate')">
                        <template #body="{ data }">
                            <Skeleton height="2rem" v-if="isLoading"/>
                            <p v-else>{{ data.locationQuantity.expiryDate ? getFormattedDate(data.locationQuantity.expiryDate) : '-' }}</p>
                        </template>
                    ...
                    <Column
                        v-if="entityType === 'TransferInvoice' || entityType === 'OrderInvoice' || entityType === 'WriteOffInvoice'"
                        field="amount" :header="t('labels.amount')"
                    >
                        <template #body="{ data }">
                            <Skeleton height="2rem" v-if="isLoading"/>
                    ...
                    <Column v-if="entityType === 'IncomeInvoice' || entityType === 'OrderInvoice'" field="amount" :header="t('labels.amount')">
                        <template #body="{ data }">
                            <Skeleton height="2rem" v-if="isLoading"/>
                            <p v-else>{{ formatCurrency(data.qty) }} {{ t(`labels.${data.product?.category?.unit?.name}`) }}</p>
                        </template>
                ...
                <div class="flex gap-2 sm:gap-4">
                        ...
                        >
                            ...
                            <Column v-if="entityType === 'Inventory'" field="factQty" :header="t('labels.factQty')">
                                <template #body="{ data }">
                                    <Skeleton height="2rem" v-if="isLoading"/>
                                    <div v-else>
                                        <div v-if="data.factQty">{{ formatCurrency(data.factQty) }}</div>
                                    </div>
                                </template>
                            </Column>
                            <Column v-if="entityType === 'IncomeInvoice' || entityType === 'OrderInvoice'" field="product" :header="t('labels.product')">
                                <template #body="{ data }">
                                    <Skeleton height="2rem" v-if="isLoading"/>
                                    <div v-else>
                                        <div v-if="data.product">{{ data.product.name }}</div>
                                    </div>
                                </template>
                            </Column>
                            <Column v-if="entityType === 'IncomeInvoice' || entityType === 'OrderInvoice'" field="color" :header="t('labels.color')">
                                <template #body="{ data }">
                                    <Skeleton height="2rem" v-if="isLoading"/>
                                    <div v-else>
                                        <div v-if="data.color">{{ data.color.name }}</div>
                                    </div>
                                </template>
                            </Column>
                            <Column v-if="entityType === 'IncomeInvoice'" field="expiryDate" :header="t('labels.expiryDate')">
                                <template #body="{ data }">
                                    <Skeleton height="2rem" v-if="isLoading"/>
                                    <div v-else>
                                        <div v-if="data.expiryDate">{{ getFormattedDate(data.expiryDate) }}</div>
                                    </div>
                                </template>
                            </Column>
                            <Column v-if="entityType === 'TransferInvoice'" field="product" :header="t('labels.product')">
                                <template #body="{ data }">
                                    <Skeleton height="2rem" v-if="isLoading"/>
                                    <div v-else>
                                        <div v-if="data.locationQuantity?.product">{{ data.locationQuantity.product.name }}</div>
                                    </div>
                                </template>
                            </Column>
                            <Column
                                v-if="entityType === 'IncomeInvoice' || entityType === 'OrderInvoice' || entityType === 'TransferInvoice'
                                 || entityType === 'ReturnInvoice' || entityType === 'WriteOffInvoice'"
                                field="amount" :header="t('labels.amount')"
                            >
                                <template #body="{ data }">
                                    <Skeleton height="2rem" v-if="isLoading"/>
                                    <div v-else>
                                        <div v-if="data.qty">{{ formatCurrency(data.qty) }}</div>
                                    </div>
                                </template>
                            </Column>
                            <Column v-if="entityType === 'IncomeInvoice' || entityType === 'OrderInvoice'" field="price" :header="t('labels.price')">
                            ...
                            <Column v-if="entityType === 'IncomeInvoice'" field="transportationFee" :header="t('labels.transportationFee')">
                                <template #body="{ data }">
                                    <Skeleton height="2rem" v-if="isLoading"/>
                                    <div v-else>
                                        <div v-if="data.transportationFee">{{ formatCurrency(data.transportationFee) }}$</div>
                                    </div>
                                </template>
                            </Column>
                            <Column v-if="entityType === 'IncomeInvoice'" field="customsFee" :header="t('labels.customsFee')">
                                <template #body="{ data }">
                                    <Skeleton height="2rem" v-if="isLoading"/>
                                    <div v-else>
                                        <div v-if="data.customsFee">{{ formatCurrency(data.customsFee) }}$</div>
                                    </div>
                                </template>
                            </Column>
                            <Column v-if="entityType === 'IncomeInvoice'" field="costPrice" :header="t('labels.costPrice')">
                                <template #body="{ data }">
                                    <Skeleton height="2rem" v-if="isLoading"/>
                                    <div v-else>
                                        <div v-if="data.costPrice">{{ formatCurrency(data.costPrice) }}$</div>
                                    </div>
                                </template>
                            </Column>
                        </DataTable>
                    ...
                    <div class="w-full mt-4">
                        ...
                        >
                            ...
                            <Column v-if="entityType === 'IncomeInvoice' || entityType === 'OrderInvoice'" :header="t('labels.color')">
                                <template #body="{ data }">
                                    <Skeleton height="2rem" v-if="isLoading"/>
                                    <div v-else>
                                        <div v-if="data.color">{{ data.color.name }}</div>
            ...
            >
                ...
                >
                    ...
                    <Column v-if="entityType === 'TransferInvoice' || entityType === 'WriteOffInvoice'" field="costPrice" :header="t('labels.costPrice')">
                        <template #body="{ data }">
                            <Skeleton height="2rem" v-if="isLoading"/>
                            <p v-else>{{ formatCurrency(data.locationQuantityKit?.kit.costPrice) }}$</p>
                        </template>
                    </Column>
                ...
                >
                    ...
                    <Column v-if="entityType === 'OrderInvoice'" field="qty" :header="t('labels.qty')">
                        <template #body="{ data }">
                            <Skeleton height="2rem" v-if="isLoading"/>
                            <p v-else>{{ formatCurrency(data.qty) }} {{ t(`labels.pcs`) }}</p>
                        </template>
                ...
                <div class="flex gap-2 sm:gap-4">
                    ...
                    <div class="w-full mt-4">
                        ...
                        >
                            ...
                            <Column v-if="entityType === 'OrderInvoice'" field="kit" :header="t('labels.kit')">
                                <template #body="{ data }">
                                    <Skeleton height="2rem" v-if="isLoading"/>
                                    <div v-else>
                                        <div v-if="data.kit">{{ data.kit.name }}</div>
                                    </div>
                                </template>
                            </Column>
                            <Column v-if="entityType === 'TransferInvoice'" field="kit" :header="t('labels.kit')">
                                <template #body="{ data }">
                                    <Skeleton height="2rem" v-if="isLoading"/>
                                    <div v-else>
                                        <div v-if="data.locationQuantityKit?.kit">{{ data.locationQuantityKit.kit.name }}</div>
                                    </div>
                                </template>
                            </Column>
                            <Column
                                v-if="entityType === 'OrderInvoice' || entityType === 'TransferInvoice'
                                || entityType === 'ReturnInvoice' || entityType === 'WriteOffInvoice'"
                                field="amount" :header="t('labels.amount')"
                            >
                                <template #body="{ data }">
                    ...
                    <div class="w-full mt-4">
                        <DataTable
                            ...
                            ref="dt"
                            :value="isLoading ? Array(10).fill({}) : refs.selectedNewChangedKits.value"
                            scrollable
                            pt:footer="border-none dark:bg-surface-800"
                            pt:root="border border-surface-300 dark:border-surface-600/50"
                        >
