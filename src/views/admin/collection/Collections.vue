...
const items = computed(() => [{ label: t("cards.collections") }]);
...
watch(
    ...
    async () => {
        ...
        await assemblyStore.fetchAssemblies(route.query);
    },
    { immediate: true, deep: true },
);

const deleteAction = (id) => {
    currentAssemblyId.value = id;
...
const deleteCustomer = async () => {
    ...
    await assemblyStore.deleteAssembly(currentAssemblyId.value);
    toast.add({ severity: 'success', summary: t('toast.deleted', { name: t('collection.nominativeCapitalize') }), life: 3000 })
...
const restoreCustomer = async () => {
    isDeleteLoading.value = true;
    await assemblyStore.restoreAssembly(currentAssemblyId.value);
    isDeleteLoading.value = false;
    visible.value.restoreVisible = false;
    toast.add({ severity: 'success', summary: t('toast.restored', { name: t('collection.nominativeCapitalize') }), life: 3000 })
};
...
<template>
    ...
    <Section
        ...
        :section-name="t('cards.collections')"
    ...
    >
        <template #buttons>
            <div class="hidden sm:flex grow gap-2 sm:gap-4 justify-end">
                ...
                />
                <Button
                    @click="router.push({ name: 'add-collection' })"
                    class="px-2 sm:px-5 whitespace-nowrap"
                >
        ...
        <template #sectionBody>
            ...
            <!-- TABLE OF USERS -->
            <Card
                v-if="assemblyStore.getIsLoadingAssembly || assemblyStore.getAssemblies.totalItems > 0"
                ...
                <template #content>
                    ...
                    >
                        <Column field="id" :header="t('labels.id')">
                            <template #body="{ data }">
                                <Skeleton height="2rem" v-if="assemblyStore.getIsLoadingAssembly"/>
                                <p v-else>{{ data.id }}</p>
                            </template>
                        </Column>
                        ...
                        <template #footer>
                            <div v-if="assemblyStore.getIsLoadingAssembly" class="flex justify-between">
            ...
            >
                    <span class="text-surface-500 dark:text-surface-400 block whitespace-nowrap">
                        {{ t('dialog.deleteConfirmation', { name: t('collection.accusative'), id: currentAssemblyId }) }}
                    </span>
            ...
            >
                    <span class="text-surface-500 dark:text-surface-400 block whitespace-nowrap">
                        {{ t('dialog.recoverConfirmation', { name: t('collection.accusative'), id: currentAssemblyId }) }}
                    </span>
