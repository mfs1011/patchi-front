...
const items = computed(() => [{ label: t('cards.collections'), route: { name: 'collections'} }, { label: t('sections.collections.edit') }]);
...
const onSubmit = handleSubmit(async values => {
    ...
    try {
        const response = await assemblyStore.putAssembly(payload, route.params.id)
        isEdited.value = true

        toast.add({ severity: 'success', summary: t('toast.edited', { name: t('collection.nominativeCapitalize') }), life: 3000 })
        ...
        toast.add({ severity: 'error', summary: t('toast.already_exists_error_named', { name: t('collection.nominativeCapitalize') }), life: 3000 })
    }
})
...
onMounted(async () => {
    ...
    await assemblyStore.fetchAssembly(route.params.id)

    isLoading.value = false

    initialValues.value = {
        name : assemblyStore.getAssembly.name,
    }
...
const isChanged = computed(() => name.value !== assemblyStore.getAssembly.name)
...
<template>
    ...
    <Section
        :section-name="t('sections.collections.edit')"
        back-route-name="collections"
        without-buttons
    >
