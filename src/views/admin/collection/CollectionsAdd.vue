...
const items = computed(() => [{ label: t('cards.collections'), route: { name: 'collections'} }, { label: t('sections.collections.add') }]);
...
const onSubmit = handleSubmit(async values => {
    ...
    try {
        const response = await assemblyStore.pushAssembly(payload)
        toast.add({ severity: 'success', summary: t('toast.created', { name: t('collection.nominativeCapitalize') }), life: 3000 })
...
<template>
    ...
    <Section
        :section-name="t('sections.collections.add')"
