...
const items = computed(() => [{ label: t("cards.shops") }]);
...
const deleteLocation = async () => {
    ...
    await locationStore.deleteLocation(currentShopId.value);
    toast.add({ severity: 'success', summary: t('toast.deleted', { name: t('shops.nominativeCapitalize') }), life: 3000 })
...
const restoreLocation = async () => {
    isDeleteLoading.value = true;
    await locationStore.restoreLocation(currentShopId.value);
    isDeleteLoading.value = false;
    visible.value.restoreVisible = false;
    toast.add({ severity: 'success', summary: t('toast.restored', { name: t('shops.nominativeCapitalize') }), life: 3000 })
};
...
<template>
    ...
    <Section
        :add-button-name="t('buttons.newShop')"
        :section-name="t('cards.shops')"
    ...
    >
        <template #buttons>
            <div class="hidden sm:flex grow gap-2 sm:gap-4 justify-end">
                ...
                />
                <Button
                    @click="router.push({ name: 'add-shop' })"
                    class="px-2 sm:px-5 whitespace-nowrap"
                >
                    {{ t("buttons.newShop") }}
                </Button>
        ...
        <template #sectionBody>
            ...
            >
                <template #content>
                    ...
                    >
                        ...
                        <Column field="name" :header="t('labels.shopName')">
                            <template #body="{ data }">
                                <Skeleton height="2rem" v-if="locationStore.getIsLoadingLocation"/>
            ...
            >
                    <span class="text-surface-500 dark:text-surface-400 block whitespace-nowrap">
                        {{ t('dialog.deleteConfirmation', { name: t('shops.accusative'), id: currentShopId }) }}
                    </span>
            ...
            >
                    <span class="text-surface-500 dark:text-surface-400 block whitespace-nowrap">
                        {{ t('dialog.recoverConfirmation', { name: t('shops.accusative'), id: currentShopId }) }}
                    </span>
