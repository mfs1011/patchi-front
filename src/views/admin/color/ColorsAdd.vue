...
const items = computed(() => [{ label: t('cards.colors'), route: { name: 'colors'} }, { label: t('sections.colors.add') }]);

// VeeValidate formani sozlash
const schema = computed(() => yup.object({
    name: yup.string().required(t('errorMessages.colorNameRequired')).max(30 , t('errorMessages.nameMustBeMaxCharacters', { count: 30 })),
}))
...
const onSubmit = handleSubmit(async values => {
    ...
    try {
        const response = await colorStore.pushColor(payload)
        resetForm()
        router.back()

        toast.add({ severity: 'success', summary: t('toast.created', { name: t('colors.nominativeCapitalize') }), life: 3000 })
        return response;
    } catch (error) {
        toast.add({ severity: 'error', summary: t('toast.already_exists_error_named', { name: locale.value === 'uz' ? t('color_name.nominative') : t('color_name.nominativeCapitalize') }), life: 3000 })
    }
})
</script>
...
<template>
    ...
    <Section
        :section-name="t('sections.colors.add')"
        back-route-name="colors"
        without-buttons
    >
            ...
            >
                <template #content>
                    <form @submit.prevent="onSubmit" class="grid grid-cols-1 sm:w-fit gap-2 sm:gap-4">
                        <label class="block">
                            <span>{{ t('labels.colorName') }}</span><span class="text-red-500"> *</span>
                            <InputText
                                v-model.trim="name"
                                fluid
                                :placeholder="t('placeholders.colorName')"
                                size="large"
